...
function OnCollectItem(node_ptr, item_ptr)
	...
	if scene_node~=nil then
		...
		local root_node = ScnMgr:getRootSceneNode()
		scene_node:setParent(root_node)
		scene_node:drop()
		scene_node:setVisible(true)

		ScnMgr:addToDeletionQueue(scene_node, 1000)
		ScnMgr:addToVisibilityChangeQueue(scene_node, false, 1000)
...
function OnUseItem(node_ptr, item_ptr)
	...
	ScnMgr:addToDeletionQueue(item_scene_node, 0)
